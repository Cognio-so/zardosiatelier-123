...
export const Route = createFileRoute("/industries")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Couture houses, bridal labels, designer menswear, accessory brands — the industries our atelier serves." },
      { property: "og:title", content: "Industries — Maison Auréline" },
      { property: "og:description", content: "Industries served by our luxury embroidery atelier." },
...
const industries = [
  ...
  { label: "Designer Womenswear", desc: "Seasonal embellishment for ready-to-wear collections.", img: p6 },
  { label: "Designer Menswear", desc: "Tonal embroidery and patch programs for tailoring.", img: p2 },
  { label: "Luxury Accessories", desc: "Embellished handbags, belts and headpieces.", img: handbag },
  { label: "Costume & Editorial", desc: "Embroidery for film, opera and editorial projects.", img: p1 },
];
          ...
          {industries.map((s, i) => (
            <Reveal key={s.label} delay={(i % 3) * 100} className="group">
              <div className="overflow-hidden aspect-[4/5] luxury-card">
                <img src={s.img} alt={s.label} loading="lazy" className="h-full w-full object-cover transition-transform duration-[1400ms] ease-[cubic-bezier(.19,1,.22,1)] group-hover:scale-[1.06]" />
              </div>
