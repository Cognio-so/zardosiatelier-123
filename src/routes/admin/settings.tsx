  ...
  return (
    <div className="admin-page space-y-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-3">{dirty && <span className="text-xs font-bold text-[#c9a44c]">Unsaved changes</span>}<button onClick={() => saveMut.mutate()} disabled={saveMut.isPending || !dirty} className="admin-primary-btn flex items-center gap-2 px-5 py-3 text-sm font-bold">{saveMut.isPending ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />} Save Changes</button></div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
        <div className="admin-glass h-fit p-3">
          <div className="space-y-1">
            {SECTIONS.map((section) => {
              ...
              return <button key={section.id} onClick={() => setActiveSection(section.id)} className={`relative flex w-full items-center gap-3 rounded-[20px] px-4 py-3 text-left text-sm font-bold transition ${active ? "text-white" : "text-slate-500 hover:bg-white/70 hover:text-slate-950"}`}>{active && <motion.span layoutId="settings-tab" className="absolute inset-0 rounded-[20px] bg-gradient-to-r from-blue-600 to-violet-600" />}<Icon size={16} className="relative" /><span className="relative">{section.label}</span></button>;
            })}
          </div>
          <a href="https://zardosiatelier-123.vercel.app/" target="_blank" rel="noopener noreferrer" className="mt-4 flex items-center gap-2 rounded-[18px] px-4 py-3 text-xs font-bold text-slate-400 transition hover:bg-white/70 hover:text-slate-950"><ExternalLink size={13} /> View Live Site</a>
        </div>
        ...
        <motion.div key={activeSection} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="admin-glass p-6">
          {isLoading ? <div className="space-y-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-14 animate-pulse rounded-[20px] bg-white/50" />)}</div> : <div className="space-y-6">
            <div className="flex items-center gap-3"><div className="admin-gradient-icon flex size-11 items-center justify-center rounded-2xl">{(() => { const Icon = currentSection.icon; return <Icon size={18} />; })()}</div><div><h3 className="text-lg font-bold text-slate-950">{currentSection.label} Settings</h3><p className="text-sm text-slate-500">Manage the values used across the public website.</p></div></div>
            <div className="grid gap-5 xl:grid-cols-2">
              {currentSection.fields.map((field) => (
                <label key={field.key} className={field.type === "textarea" || field.type === "toggle" ? "xl:col-span-2" : ""}>
                  <span className="admin-label mb-2 block">{field.label}</span>
                  {field.type === "textarea" ? <textarea value={(formData[field.key as keyof SiteSettings] as string) ?? ""} onChange={(e) => update(field.key, e.target.value)} placeholder={field.placeholder} rows={4} className="admin-input w-full resize-none px-4 py-3 text-sm" /> : field.type === "toggle" ? <div className="rounded-[24px] border border-white/80 bg-white/60 p-5"><button type="button" onClick={() => update(field.key, !(formData[field.key as keyof SiteSettings] as boolean))} className={`relative h-8 w-14 rounded-full transition ${(formData[field.key as keyof SiteSettings] as boolean) ? "bg-gradient-to-r from-blue-600 to-violet-600" : "bg-slate-200"}`}><span className="absolute top-1 size-6 rounded-full bg-white shadow transition" style={{ left: (formData[field.key as keyof SiteSettings] as boolean) ? "26px" : "4px" }} /></button><span className="ml-4 text-sm font-semibold text-slate-600">{(formData[field.key as keyof SiteSettings] as boolean) ? "Maintenance mode is ON" : "Site is live and visible"}</span></div> : <input type={field.type} value={(formData[field.key as keyof SiteSettings] as string) ?? ""} onChange={(e) => update(field.key, e.target.value)} placeholder={field.placeholder} className="admin-input w-full px-4 py-3 text-sm" />}
                </label>
              ))}
            </div>
          </div>}
